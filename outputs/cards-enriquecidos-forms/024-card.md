...
[ ] A - Use separate extraction calls for each field to ensure consistent handling of each type.B.Extract data as-is and normalize formats in post-processing code after Claude returns.C.Request multiple extraction attempts per document and select the most common format.D.Define a strict output schema and include format normalization rules in your prompt.
[ ] B - Extract data as-is and normalize formats in post-processing code after Claude returns.C.Request multiple extraction attempts per document and select the most common format.D.Define a strict output schema and include format normalization rules in your prompt.
[ ] C - Request multiple extraction attempts per document and select the most common format.D.Define a strict output schema and include format normalization rules in your prompt.
