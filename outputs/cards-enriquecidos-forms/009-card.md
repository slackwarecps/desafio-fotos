...
[ ] A - The verification tool is clearing the agent's internal state after each successful validation step.B.The prompt lacks instructions telling Claude to remember information across multiple exchanges.C.The conversation history isn't being passed in subsequent API requests.D.Claude's memory retention is limited to two conversational turns by default, requiring explicit configuration to extend it.
[ ] B - The prompt lacks instructions telling Claude to remember information across multiple exchanges.C.The conversation history isn't being passed in subsequent API requests.D.Claude's memory retention is limited to two conversational turns by default, requiring explicit configuration to extend it.
[ ] C - The conversation history isn't being passed in subsequent API requests.D.Claude's memory retention is limited to two conversational turns by default, requiring explicit configuration to extend it.
