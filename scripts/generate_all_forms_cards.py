#!/usr/bin/env python3
"""
Full Automated Generator for Enriched Flashcards from formulario.tsv
Processes all 60 questions in continuous single-pass workflow.
...
json_path = os.path.join(OUTPUT_DIR, "questions-parsed.json")
if not os.path.exists(json_path):
    print("❌ questions-parsed.json não encontrado!")
    sys.exit(1)

with open(json_path, "r", encoding="utf-8") as f:
    questions = json.load(f)

# Comprehensive high-quality dataset of answer keys, translations, and deep explanations for all 60 questions
# Built specifically around Claude Certified Architect exam patterns
KNOWLEDGE_BASE = {
    1: {
        "correct": "D",
        ...
        "tech": {
            "intro": "Esta questão avalia estratégias de manipulação de arquivos quando ferramentas de edição por substituição pontual (`Edit`) falham devido à ambiguidade no código-fonte.",
...
print("Generator dataset helper ready.")
